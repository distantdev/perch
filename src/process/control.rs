...
impl ProcessControl {
    pub fn resolve_pid(
        ...
        if let Ok(port) = target.parse::<u16>() {
            ...
            if let Some(server) = scan.servers.iter().find(|s| s.port == port) {
                return Ok(server.pid);
            }
            return Err(PerchError::PortNotFound(port));
        }
    ...
    pub fn kill(
        ...
        let pid = Self::resolve_pid(target, scanner, config)?;
        ensure_can_control(pid)?;
        if force {
            platform_kill_force(pid)
        } else {
...
fn ensure_can_control(pid: u32) -> Result<()> {
    #[cfg(windows)]
    {
        ...
        if !can_open_process(pid) {
            return Err(PerchError::PermissionDenied {
                pid,
                reason: "unable to open process; try running as administrator".into(),
            });
        }
        Ok(())
    }
...
#[cfg(windows)]
fn can_open_process(pid: u32) -> bool {
